...
//! NON È EFFICIENTE, posso fare una funzione costante che fa il return della serie (n(n+1))/2
int A(int n){
    int s = 0;
...
int foo(int n){
    if(n <= 2)
        return 1;
    else if(n > 321){ //? considero solo questo if (comportamento asintotico, per n sufficientemente grande)
        int i = n/3;
        return 2*foo(i)+n*n*n*i; //? considero solo foo(i), le altre sono costanti che non mi dicono nulla dopo la chiamata ric
    }else
...
//! T_fun(n)= 2*T_fun(n/2)+Teta(n*sqrt(n)) = ... [t. master - 3° caso] ... = Teta(n+sqrt(n))
int fun(int *A, int n){
    if( n < 1)
        return 1;
    else{
        int t = fun(A, n/2);
        if(t > A[n]) //? lo considero perché worst case dato che fa una chiamata ric
            t = t + fun(A, n/2);
        for(int j = 0; j < n; j++)
            t = t + A[j] + proc(n);
        return t;
