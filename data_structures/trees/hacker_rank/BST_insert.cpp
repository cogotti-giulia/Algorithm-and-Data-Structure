  ...
  Node *insert(Node *root, int data) {
    ...
    while (scorri != nullptr) {
      ...
      if (node->data < scorri->data)
        ...
        allora devo andare nel sotto albero sinistro, altrimenti nel destro*/
        scorri = scorri->left;
      else
        scorri = scorri->right;
    }

    // a questo punto y contiene il nodo foglia a cui attaccare il nuovo nodo
    // se l'albero fosse vuoto, allora il nuovo nodo è la radice dell'albero
    if (y == nullptr) {
      root = node;
      return root;
    } else {
      /*altrimenti in base al valore della chiave lo inserisco a sinistra o a
       * destra*/
      if (node->data < y->data)
        y->left = node;
      else
