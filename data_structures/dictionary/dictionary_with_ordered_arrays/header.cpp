...
/********************************************** reallocate *************************************************/
//TODO: fare una reallocate efficiente con la tecnica di raddoppiamento e dimezzamento
//! T_reallocate(n) = O(n)
void reallocate(my_array A, size_t new_dim, size_t* size){
    ...
    for(int i = 0; i < std::min(new_dim, *size); i++){ //? viene eseguito n volte
        /*tmp_A[i].key = A[i].key;
        tmp_A[i].info = A[i].info;*/
        tmp_A[i] = A[i];
    }

    *size = new_dim;//la dimensione sarà quella del nuovo array (potrebbe non essere +1 anche se qua lo è)
...
/********************************************** insert *************************************************/
//! T_insert(n) = Teta(1) + i*d + O(n) + (n-i+1)*d = Teta(1) + (n+1)d + O(n) = O(n)
//! d è il costo dell'assegnamento del corpo del ciclo
//! Teta(1) insieme di tutte le operazioni costanti
void insert(my_array A, int k, int v, size_t* size){
    int i = 0;
    //scorro fino a trovare la posizione nella quale la chiave è minore di quella da inserire
    //oppure finché non finisco l'array
...
//! (n-i)*d = O(n) perché se ho fortuna elimino quello in ultima posizione, ma, nel caso peggiore devo scorrere tutto l'array!
