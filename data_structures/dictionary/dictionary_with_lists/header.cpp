...
/*********************************** insert *********************************/
//! T_insert(n) = Teta(1) costante, inserisco SEMPRE in testa (possibili duplicati)
...
elem search(L list, int k){
    ...
    while(scorri->next != nullptr && scorri->key != k)//? al più O(n), vedi INVARIANTE negli appunti :(
        scorri = scorri->next;
...
void delete_(L list, int k){
    ...
    while(scorri != nullptr){ //? eseguito n volte, devo scorrere tutto perché possono esserci duplicati
        if(scorri->key == k){
            if(scorri->next != nullptr)
